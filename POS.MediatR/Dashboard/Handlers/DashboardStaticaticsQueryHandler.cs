...
namespace POS.MediatR.Handlers
{
    public class DashboardStaticaticsQueryHandler : IRequestHandler<DashboardStaticaticsQuery, DashboardStatics>
    {
        ...
        public async Task<DashboardStatics> Handle(DashboardStaticaticsQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                TotalPurchase = await _purchaseOrderRepository.All.Where(c => c.SupplierId == SuppliersId).SumAsync(c => c.TotalAmount);
               ...
               .Where(c => c.Status == PurchaseSaleItemStatusEnum.Return && c.PurchaseOrder.SupplierId == SuppliersId)
            ...
            TotalSales = await _salesOrderRepository.All.Where(a => a.ProductMainCategoryId== request.ProductMainCategoryId).SumAsync(c => c.TotalAmount);
            ...
            TotalSalesReturn = await _salesOrderItemRepository.AllIncluding(c => c.SalesOrder)
                .Where(c => c.Status == PurchaseSaleItemStatusEnum.Return && c.SalesOrder.ProductMainCategoryId==request.ProductMainCategoryId)                
                .SumAsync(c => (c.UnitPrice * c.Quantity) + c.TaxValue - c.Discount);
